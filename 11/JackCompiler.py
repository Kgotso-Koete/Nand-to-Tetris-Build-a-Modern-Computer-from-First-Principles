...
def parser_driver():
    ...
    if not JackParser.IS_directory:  
        ...
        # initialising VM code writer I/O
        JackParser.VMwriter.input_file_path = sys.argv[1]
        JackParser.VMwriter.output_file_path = sys.argv[1][:-5] + '.vm' 
        JackParser.VMwriter.current_file_read =JackParser.input_file_path.split('/')[-1]
        # tokenize 
    ...
    if JackParser.IS_directory:
        for file_name in sorted(os.listdir(JackParser.cmd_path)):
            if file_name.endswith(".jack"): 
                ...
                JackParser.current_file_read = JackParser.input_file_path.split('/')[-1]
                # initialising VM code writer I/O
                JackParser.VMwriter.input_file_path = JackParser.cmd_path + '/' + file_name 
                JackParser.VMwriter.output_file_path = JackParser.cmd_path + '/' + file_name[0:-5] + '.vm'
                JackParser.VMwriter.current_file_read = JackParser.input_file_path.split('/')[-1]
                # tokenize     
                JackParser.get_token_list()  

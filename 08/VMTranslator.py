# Uses python3
import re

class VMTranslator:

    def __init__(self):
        # I/O data structures
        self.path = None 
        self.IS_directory = None
        self.file_list = []
        self.input_file = None # input file used to parse commands
        self.output_file = None  # output file where binary code will be written
        self.current_file_read = None #specify current file being read for static variable alloc

        # key data structures
        self.input_data = None  # commands parsed and captured in list format
        self.standard_VMcommand_table = None # standard VM commands and their types 
        self.standard_VMtosymbol = None # standard VM memory command to assembly symbol map
        self.output_dict = None # output dictionary that will store the translation    
        self.DIR_outputs = None # list of output dictionaries for directories   
        
    def parser(self, input_file):
        """
        Purpose: Unpacks each underlying instructin into it's underlying fields
        Input:Text file containing instuctions
        Output: Data structure containing fields
        """
        self.input_data = []
        # open file
        with open(input_file, 'r') as f:
            source = [line.rstrip() for line in f]   
            for cmd in source:
                if not cmd.startswith('//') and not cmd == '': 
                    cmd = cmd.partition('/')[0] 
                    self.input_data.append(cmd.split()) 
        return 
    
    def initialise_output_dictionary(self, input_data):
        """
        Purpose: Stores the input data into an output dictionary
        Input: List containing instructions
        Output: Dictionary with various fields
        """
        self.output_dict = {}
        for i in range(len(input_data)):
            self.output_dict[i] = input_data[i]
        return
    
    def initialise_standard_VMcommand_table(self):
        """
        Purpose: Stores the standard commands and their types of the VM
        Input: An empty dictionary
        Output: Dictionary with various fields
        """
        self.standard_VMcommand_table = {'add': 'C_ARITHMETIC', 'sub': 'C_ARITHMETIC', 'neg': 'C_ARITHMETIC', 'eq': 'C_ARITHMETIC',
                                         'gt': 'C_ARITHMETIC', 'lt': 'C_ARITHMETIC','and': 'C_ARITHMETIC','or': 'C_ARITHMETIC',
                                         'not': 'C_ARITHMETIC','pop': 'C_POP', 'push': 'C_PUSH', 'label': 'C_LABEL', 'goto': 'C_GOTO', 
                                         'if-goto': 'C_IF','function': 'C_FUNCTION', 'call':'C_CALL','return':'C_RETURN'}    
              
        return
    
    
    def initialise_standard_VMtosymbol(self):
        """
        Purpose: Set up the standard VM memory command to assembly symbol map
        Input: Uninitialized standard VM to asm symbol map
        Output: Initialized standard VM to asm symbol map
        """
        self.standard_VMtosymbol = {'local': 'LCL', 'argument': 'ARG', 'this': 'THIS', 'that': 'THAT'}
        return

    def initialise_output_dict(self, input_data):
        """
        Purpose: Initialise the output dictionary
        Input: Uninitialised output dictionary
        Output: Output dictionary initialised with commands 
        """
        self.output_dict = {}
        for i in range(len(input_data)):
            self.output_dict[i] = {}
            self.output_dict[i]['VM code'] = ''
            self.output_dict[i]['VM cmd type'] = '' 
            self.output_dict[i]['assembly code'] = '' 

        return 
    
    def get_VMcode_type(self, input_data):
        """
        Purpose: Load VM commmand types in the output dictionary with each command
        Input: An empty output dictionary
        Output: Output dictionary initialised with commands and their type
        """
        for i in range(len(input_data)):
            self.output_dict[i]['VM code'] = input_data[i]
            VM_cmd = self.output_dict[i]['VM code'][0]
            self.output_dict[i]['VM cmd type'] = self.standard_VMcommand_table[VM_cmd]   

        return
    
    def code_C_PUSH(self, i, VM_code, command):
        """
        Purpose: Converts single 'push' VM commands into assembly code
        Input: Ouptut dictionary containing 'push' VM commands and their type
        Output: Output dictionary updated with 'push' assembly transaltions 
        """

        segment = VM_code[1] # local, constant, argument? etc
        index = VM_code[2] # index to be used in push/pop the value of segment[index] in stack
        assembly = ''    

        if segment in self.standard_VMtosymbol:
            asm_symbol = self.standard_VMtosymbol[segment] 
            # addr = segment_pointer + index, *SP = *addr, SP ++
            add_index = ['@' + index,'D=A'] # load into index into the D register
            addr = ['@' + asm_symbol, 'M=M+D', 'A=M'] # addr = segment_pointer + index 
            data_to_push = ['D=M'] # *addr 
            SP_move = ['@SP', 'A=M', 'M=D'] # *SP = *addr 
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            minus_index = ['@' + index,'D=A'] # load into index into the D register 
            reset_addr = ['@' + asm_symbol, 'M=M-D'] # addr = segment_pointer + index  
            assembly = add_index + addr + data_to_push + SP_move + SP_increment + minus_index + reset_addr
            return assembly 
        
        if  segment == 'constant':
            # addr = constant, *SP = *addr, SP ++
            addr = ['@' + index] # addr = constant
            data_to_push = ['D=A'] # *addr  
            SP_move = ['@SP', 'A=M', 'M=D'] # *SP = *addr 
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = addr + data_to_push + SP_move + SP_increment
            return assembly  
        
        if  segment == 'temp':
            # addr = segment_pointer + index, *SP = *addr, SP ++
            addr = ['@R' + str(5 + int(index))] # addr = segment_pointer + index   
            data_to_push = ['D=M'] # *addr 
            SP_move = ['@SP', 'A=M', 'M=D'] # *SP = *addr 
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = addr + data_to_push + SP_move + SP_increment
            return assembly 
        
        if segment == 'pointer' and index == '0':  
            # *SP = THIS/THAT,SP++
            data_to_push = ['@THIS','D=M'] # THIS    
            SP_val = ['@SP', 'A=M', 'M=D'] # *SP 
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = data_to_push + SP_val + SP_increment
            return assembly
        
        if segment == 'pointer' and index == '1': 
            # *SP = THIS/THAT,SP++
            data_to_push = ['@THAT','D=M'] # THAT      
            SP_val = ['@SP', 'A=M', 'M=D'] # *SP 
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = data_to_push + SP_val + SP_increment 
            return assembly
        
        if segment == 'static':  
            data_to_push = ['@' + self.current_file_read + '.' + index, 'D=M'] 
            SP_val = ['@SP', 'A=M', 'M=D'] # *SP 
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = data_to_push + SP_val + SP_increment 
            return assembly
 
        return
    
    def code_C_POP(self, i, VM_code, command):
        """
        Purpose: Converts single 'pop' VM commands into assembly code
        Input: Ouptut dictionary containing 'pop' VM commands and their type
        Output: Output dictionary updated with 'pop' assembly transaltions 
        """

        segment = VM_code[1] # local, constant, argument? etc
        index = VM_code[2] # index to be used in push/pop the value of segment[index] in stack
        assembly = ''

        if segment in self.standard_VMtosymbol:
            asm_symbol = self.standard_VMtosymbol[segment] 
            # addr = segment_pointer + index, SP--, *addr = *SP
            add_index = ['@' + index,'D=A'] # load into index into the D register 
            addr = ['@' + asm_symbol, 'M=M+D'] # addr = segment_pointer + index 
            SP_decrement = ['@SP','M=M-1','A=M'] # SP -- 
            data_to_push = ['D=M'] # *SP 
            segment_change = ['@' + asm_symbol,'A=M', 'M=D'] # *addr = *SP 
            minus_index = ['@' + index,'D=A'] # load into index into the D register 
            reset_addr = ['@' + asm_symbol, 'M=M-D'] # addr = segment_pointer + index  
            reset_pointer_memory = ['@SP','A=M','M=0']  
            assembly = add_index + addr + SP_decrement + data_to_push + segment_change + minus_index + reset_addr + reset_pointer_memory
            return assembly    
        
        if segment == 'temp':
            # addr = segment_pointer + index, SP--, *addr = *SP 
            SP_decrement = ['@SP','M=M-1','A=M'] # SP -- 
            data_to_push = ['D=M'] # *SP 
            segment_change = ['@R' + str(5 + int(index)), 'M=D'] # *addr = *SP 
            reset_pointer_memory = ['@SP','A=M','M=0']
            assembly = SP_decrement + data_to_push + segment_change + reset_pointer_memory
            return assembly  
        
        if segment == 'pointer' and index == '0':  
            # *SP--,THIS/THAT = *SP
            SP_decrement = ['@SP','M=M-1','A=M'] # SP -- 
            data_to_push = ['D=M'] # *SP
            segment_change = ['@THIS', 'M=D'] # THIS/THAT = *SP
            reset_pointer_memory = ['@SP','A=M','M=0']  
            assembly = SP_decrement + data_to_push + segment_change + reset_pointer_memory
            return assembly
        
        if segment == 'pointer' and index == '1':  
            # *SP--,THIS/THAT = *SP
            SP_decrement = ['@SP','M=M-1','A=M'] # SP -- 
            data_to_push = ['D=M'] # *SP
            segment_change = ['@THAT', 'M=D'] # THIS/THAT = *SP
            reset_pointer_memory = ['@SP','A=M','M=0']           
            assembly = SP_decrement + data_to_push + segment_change + reset_pointer_memory
            return assembly
          
        if segment == 'static': 
            SP_decrement = ['@SP','M=M-1','A=M'] # SP -- 
            data_to_push = ['D=M'] # *SP
            segment_change = ['@' + self.current_file_read + '.' + index, 'M=D'] # THIS/THAT = *SP  
            reset_pointer_memory = ['@SP','A=M','M=0']            
            assembly = SP_decrement + data_to_push + segment_change + reset_pointer_memory
            return assembly
 

        return
    
    def code_C_ARITHMETIC(self, i, VM_code, command):
        """
        Purpose: Converts single 'arithmetic' VM commands into assembly code
        Input: Ouptut dictionary containing 'arithmetic' VM commands and their type
        Output: Output dictionary updated with 'arithmetic' assembly transaltions 
        """

        arithmetic_command = VM_code[0] # add, subtract? etc
        assembly = ''
        

        if arithmetic_command == 'add':
            # SP --, pop *SP, SP--, pop 
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop
            reset_pointer_memory1 = ['@SP','A=M','M=0']
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D+M'] # n2 = pop
            to_push = ['@SP', 'A=M','M=D','@SP','M=M+1']     
            assembly = SP_decrement_1 + n_1 + reset_pointer_memory1 + SP_decrement_2 + n_2 + to_push 
            return assembly 
        
        if arithmetic_command == 'sub':
             # SP --, pop *SP, SP--, pop 
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop 
            neg = ['@0', 'D=A-D'] # TO DELETE ADFTER TESTING 
            reset_pointer_memory1 = ['@SP','A=M','M=0']
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D+M'] # n2 = pop      
            to_push = ['@SP', 'A=M','M=D','@SP','M=M+1']     
            assembly = SP_decrement_1 + n_1 + neg + reset_pointer_memory1 + SP_decrement_2 + n_2 + to_push
            return assembly
        
        if arithmetic_command == 'neg':
            SP_decrement = ['@SP','M=M-1'] # SP--
            n = ['A=M','D=M'] # n1 = pop 
            neg = ['@0', 'D=A-D'] # TO DELETE ADFTER TESTING
            change = ['@SP','A=M', 'M=D']  
            SP_increment = ['@SP','M=M+1'] # SP ++
            assembly = SP_decrement + n + neg + change + SP_increment
            return assembly 
        
        if arithmetic_command == 'eq': 
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop 
            reset_pointer_memory1 = ['@SP','A=M','M=0']
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D-M'] # n2 = pop 
            jmp_cond = ['@TRUE_COND' + str(i),'D;JEQ']       
            false_cond = ['@SP','A=M', 'M=0']   
            SP_increment1 = ['@SP','M=M+1'] # SP ++   
            skip1 = ['D=0', '@SKIP' + str(i), 'D;JEQ']  
            true_cond = ['(TRUE_COND' + str(i) + ')','@SP', 'A=M', 'M=-1'] 
            SP_increment2 = ['@SP','M=M+1'] # SP ++  
            skip2 = ['(SKIP'+ str(i) + ')']        
            assembly = SP_decrement_1 + n_1 + reset_pointer_memory1 + SP_decrement_2 + n_2 + jmp_cond + false_cond + SP_increment1 + skip1 + true_cond + SP_increment2 + skip2
            return assembly
        
        if arithmetic_command == 'gt':
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop 
            reset_pointer_memory1 = ['@SP','A=M','M=0']
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D-M'] # n2 = pop 
            jmp_cond = ['@TRUE_COND' + str(i),'D;JLT']       
            false_cond = ['@SP','A=M', 'M=0']   
            SP_increment1 = ['@SP','M=M+1'] # SP ++   
            skip1 = ['D=0', '@SKIP' + str(i), 'D;JEQ']  
            true_cond = ['(TRUE_COND' + str(i) + ')','@SP', 'A=M', 'M=-1'] 
            SP_increment2 = ['@SP','M=M+1'] # SP ++  
            skip2 = ['(SKIP'+ str(i) + ')']        
            assembly = SP_decrement_1 + n_1 + reset_pointer_memory1 + SP_decrement_2 + n_2 + jmp_cond + false_cond + SP_increment1 + skip1 + true_cond + SP_increment2 + skip2
            return assembly       

        if arithmetic_command == 'lt':
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop 
            reset_pointer_memory1 = ['@SP','A=M','M=0']
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D-M'] # n2 = pop 
            jmp_cond = ['@TRUE_COND' + str(i),'D;JGT']        
            false_cond = ['@SP','A=M', 'M=0']   
            SP_increment1 = ['@SP','M=M+1'] # SP ++   
            skip1 = ['D=0', '@SKIP' + str(i), 'D;JEQ']  
            true_cond = ['(TRUE_COND' + str(i) + ')','@SP', 'A=M', 'M=-1'] 
            SP_increment2 = ['@SP','M=M+1'] # SP ++  
            skip2 = ['(SKIP'+ str(i) + ')']        
            assembly = SP_decrement_1 + n_1 + reset_pointer_memory1 + SP_decrement_2 + n_2 + jmp_cond + false_cond + SP_increment1 + skip1 + true_cond + SP_increment2 + skip2
            return assembly    
        
        if arithmetic_command == 'and':
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop 
            reset_pointer_memory1 = ['@SP','A=M','M=0'] 
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D&M'] # n2 = pop
            change = ['@SP','A=M', 'M=D']   
            SP_increment = ['@SP','M=M+1'] # SP ++
            assembly = SP_decrement_1 + n_1 + reset_pointer_memory1 + SP_decrement_2 + n_2 + change + SP_increment
            return assembly 

        if arithmetic_command == 'or':
            SP_decrement_1 = ['@SP','M=M-1'] # SP--
            n_1 = ['A=M','D=M'] # n1 = pop 
            reset_pointer_memory1 = ['@SP','A=M','M=0']
            SP_decrement_2 = ['@SP','M=M-1'] # SP--
            n_2 = ['A=M','D=D|M'] # n2 = pop  
            change = ['@SP','A=M', 'M=D']  
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = SP_decrement_1 + n_1 + reset_pointer_memory1 + SP_decrement_2 + n_2 + change + SP_increment
            return assembly  
           
        if arithmetic_command == 'not':
            SP_decrement = ['@SP','M=M-1'] # SP--
            n = ['A=M','D=M'] # n1 = pop 
            neg = ['@0', 'D=!D'] # TO DELETE ADFTER TESTING
            change = ['@SP','A=M', 'M=D']  
            SP_increment = ['@SP','M=M+1'] # SP ++ 
            assembly = SP_decrement + n + neg + change + SP_increment  
            return assembly   
                   
        return 
    
    def code_C_LABEL(self, i, VM_code, command):
        """
        Purpose: Converts single 'label' VM commands into assembly code
        Input: Ouptut dictionary containing 'label' VM commands and their type
        Output: Output dictionary updated with 'label' assembly transaltions 
        """

        label_command = VM_code[0] # label, goto, if-goto etc?
        label_to_use = VM_code[1] # label to be used in asm code
        assembly = ''
        if label_command == 'label':
            assembly = ['(' + label_to_use + ')']  
            return assembly
        return
        
    def code_C_GOTO(self, i, VM_code, command):
        """
        Purpose: Converts single 'goto' VM commands into assembly code
        Input: Ouptut dictionary containing 'goto' VM commands and their type
        Output: Output dictionary updated with 'goto' assembly transaltions 
        """

        label_command = VM_code[0] # label, goto, if-goto etc?
        label_to_use = VM_code[1] # label to be used in asm code
        assembly = ''

        assembly = ['@' + label_to_use, '0;JMP']     
        return assembly
        return 
    
    def code_C_IF(self, i, VM_code, command): 
        """
        Purpose: Converts single 'goto' VM commands into assembly code
        Input: Ouptut dictionary containing 'goto' VM commands and their type
        Output: Output dictionary updated with 'goto' assembly transaltions 
        """

        label_command = VM_code[0] # label, goto, if-goto etc?
        label_to_use = VM_code[1] # label to be used in asm code
        assembly = ''
        SP_zeroANDdecrement = ['@SP','A=M', 'M=0'] # SP ++ 

        data_to_push = ['@SP','M=M-1', 'A=M', 'D=M'] # *SP    
        jmp_cond = ['@' + label_to_use,'D;JNE']         
        assembly = data_to_push + jmp_cond + SP_zeroANDdecrement  
        return assembly 
    
    def code_C_FUNCTION(self, i, VM_code, command):
        """
        Purpose: Converts single 'function' VM commands into assembly code
        Input: Ouptut dictionary containing 'function' VM commands and their type
        Output: Output dictionary updated with 'function' assembly transaltions 
        """

        label_command = VM_code[0] # label, goto, if-goto etc?
        function_name = VM_code[1] # label to be used in asm code
        n_vars = VM_code[2]
        local_var_declaration = []
        assembly = ''

        
        label_declaration = ['(' + function_name + ')'] # declare the label
        for j in range(int(n_vars)): 
            local_var_declaration += ['@0', 'D=A', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1']  # init local vars
        assembly =  label_declaration + local_var_declaration 
        return assembly    
        
    
    def code_C_RETURN(self, i, VM_code, command):
        """
        Purpose: Converts single 'return' VM commands into assembly code
        Input: Ouptut dictionary containing 'return' VM commands and their type
        Output: Output dictionary updated with 'return' assembly transaltions 
        """
    
        #endFrame = LCL
        LCL_address = ['@LCL','D=M'] # get address from LCL 0
        pop_to_endFrame = ['@R13','M=D']
        endFrame_code = LCL_address + pop_to_endFrame

        # retAddr = *(endframe - 5)
        endframeMINUS_5 = ['@R13', 'D=M', '@5', 'D=D-A'] # endframe - 5
        pop_into_retAddr = ['@SP', 'A=D', 'D=M', '@R14', 'M=D'] 
        retAddr_code = endframeMINUS_5 + pop_into_retAddr  
        
        # *ARG = POP()
        choose_ARG = ['@0','D=A', '@ARG', 'M=M+D'] # choose ARG 0 segment
        data_for_ARG = ['@SP', 'M=M-1', 'A=M', 'D=M', 'M=0'] # POP()
        pop_into_ARG = ['@ARG','A=M', 'M=D'] # *ARG
        reset_ARG_index = ['@0','D=A', '@ARG', 'M=M-D'] # reset ARG 0 segment
        reset_SP_from_ARG = ['@R13', 'D=M', '@SP', 'M=D'] 
        ARG_code = choose_ARG + data_for_ARG + pop_into_ARG + reset_ARG_index + reset_SP_from_ARG
        
        # SP = ARG + 1
        choose_ARG = ['@0','D=A', '@ARG', 'M=M+D']
        ARGplus1 = ['@ARG', 'D=M', '@1', 'D=D+A'] # ARG + 1
        new_SP_pos = ['@SP','M=D'] 
        reset_ARG_index = ['@0','D=A', '@ARG', 'M=M-D'] # reset ARG 0 segment
        fix_SP_pos = ['@SP', 'M=M'] 
        reset_SP_pos_code = choose_ARG + ARGplus1 + new_SP_pos + reset_ARG_index + fix_SP_pos

        # THAT = * (endframe - 1) 
        choose_THAT = ['@0','D=A', '@THAT', 'M=M+D']
        endframeMINUS_1 = ['@R13', 'D=M', '@1', 'D=D-A'] # endframe - 1
        pop_into_THAT = ['@SP', 'A=D', 'D=M', '@THAT', 'M=D'] 
        reset_THAT_index = ['@0','D=A', '@THAT', 'M=M-D'] 
        THAT_reset_SP_pos = ['@SP', 'M=M'] 
        THAT_code = choose_THAT + endframeMINUS_1 + pop_into_THAT + reset_THAT_index + THAT_reset_SP_pos

        # THIS = * (endframe - 2) 
        choose_THIS = ['@0','D=A', '@THIS', 'M=M+D']
        endframeMINUS_2 = ['@R13', 'D=M', '@2', 'D=D-A'] # endframe - 2 
        pop_into_THIS = ['@SP', 'A=D', 'D=M', '@THIS', 'M=D'] 
        reset_THIS_index = ['@0','D=A', '@THIS', 'M=M-D']  
        THIS_reset_SP_pos = ['@SP', 'M=M']  
        THIS_code = choose_THIS + endframeMINUS_2 + pop_into_THIS + reset_THIS_index + THIS_reset_SP_pos
        
        # ARG = * (endframe - 3) 
        choose_ARG_2 = ['@0','D=A', '@ARG', 'M=M+D']
        endframeMINUS_3 = ['@R13', 'D=M', '@3', 'D=D-A'] # endframe - 3  
        pop_into_ARG2 = ['@SP', 'A=D', 'D=M', '@ARG', 'M=D'] 
        reset_ARG2_index = ['@0','D=A', '@ARG', 'M=M-D']  
        ARG2_reset_SP_pos = ['@SP', 'M=M']  
        ARG2_code = choose_ARG_2 + endframeMINUS_3 + pop_into_ARG2 + reset_ARG2_index + ARG2_reset_SP_pos
    
        # LCL = * (endframe - 4) 
        choose_LCL_2 = ['@0','D=A', '@LCL', 'M=M+D']
        endframeMINUS_4 = ['@R13', 'D=M', '@4', 'D=D-A'] # endframe - 4  
        pop_into_LCL2 = ['@SP', 'A=D', 'D=M', '@LCL', 'M=D'] 
        reset_LCL2_index = ['@0','D=A', '@LCL', 'M=M-D']  
        LCL2_reset_SP_pos = ['@SP', 'M=M']     
        LCL2_code = choose_LCL_2 + endframeMINUS_4 + pop_into_LCL2 + reset_LCL2_index + LCL2_reset_SP_pos
          
        # goto retAddr 
        goto_retAddr = ['@R14', 'A=M','0;JMP']      
         
        return endFrame_code + retAddr_code + ARG_code + reset_SP_pos_code + THAT_code + THIS_code + ARG2_code + LCL2_code + goto_retAddr
    
    def code_C_CALL(self, i, VM_code, command):
        """
        Purpose: Converts single 'call' VM commands into assembly code
        Input: Ouptut dictionary containing 'call' VM commands and their type
        Output: Output dictionary updated with 'call' assembly transaltions 
        """
        label_command = VM_code[0] # call?
        function_name = VM_code[1] # label to be used in asm code
        nArgs = VM_code[2]

        caller_retAddr_push = ['@' + function_name + '$' + 'caller_retaddr' + str(i) , 'D=A', '@SP', 'A=M', 'M=D', '@SP','M=M+1']
        caller_push_LCL = ['@LCL', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1'] # saves LCL of the caller 
        caller_push_ARG = ['@ARG', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1'] # saves ARG of the caller
        caller_push_THIS = ['@THIS', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1'] # saves THIS of the caller
        caller_push_THAT = ['@THAT', 'D=M', '@SP', 'A=M', 'M=D', '@SP', 'M=M+1'] # saves THIS of the caller  
        caller_ARG_reposition = ['@SP', 'D=M', '@5', 'D=D-A', '@' + nArgs, 'D=D-A', '@ARG', 'M=D']
        caller_LCL_reposition = ['@SP', 'D=M', '@LCL', 'M=D']   
        goto_function = self.code_C_GOTO(i, ['goto', function_name ], 'C_GOTO')
        caller_retAddr_declaration = ['(' + function_name + '$' + 'caller_retaddr' + str(i) + ')']  
        assembly = caller_retAddr_push + caller_push_LCL + caller_push_ARG + caller_push_THIS + caller_push_THAT + caller_ARG_reposition + caller_LCL_reposition + goto_function + caller_retAddr_declaration
        return assembly   
  
        return 
                
    def code_writer(self, i):
        """
        Purpose: Converts single VM commands into assembly code
        Input: Ouptut dictionary containing VM commands and their type
        Output: Output dictionary updated with assembly transaltions
        """
        
        VM_code = self.output_dict[i]['VM code'] # entire VM code command
        command = self.output_dict[i]['VM cmd type'] # C-PUSH, C_POP or C_ARITHMETIC etc?
        
        if command == 'C_PUSH': return self.code_C_PUSH(i, VM_code, command) # PUSH?
        if command == 'C_POP': return self.code_C_POP(i, VM_code, command) # POP?
        if command == 'C_ARITHMETIC': return self.code_C_ARITHMETIC(i, VM_code, command) # C_ARITHMETIC? 
        if command == 'C_LABEL': return self.code_C_LABEL(i, VM_code, command) # C_LABEL? 
        if command == 'C_GOTO': return self.code_C_GOTO(i, VM_code, command) # C_GOTO? 
        if command == 'C_IF': return self.code_C_IF(i, VM_code, command) # C_GOTO?
        if command == 'C_FUNCTION': return self.code_C_FUNCTION(i, VM_code, command) # C_FUNCTION?
        if command == 'C_RETURN': return self.code_C_RETURN(i, VM_code, command) # C_RETURN?
        if command == 'C_CALL': return self.code_C_CALL(i, VM_code, command) # C_CALL?
        
        return 
    
    def translate(self):
        """
        Purpose: Converts all VM commands into assembly code
        Input: Ouptut dictionary containing VM commands and their type
        Output: Output dictionary updated with assembly transaltions
        """

        for i in range(len(self.input_data)):
            self.output_dict[i]['assembly code'] = self.code_writer(i) 

        return
    

    def write_asm_output(self):  
        """
        Purpose: Write asm translation to output file
        Input: Output dictionary with asm translation fields, specified output file name
        Output: Asm written to prog.asm output file
        """
        SP_init = ['@256', 'D=A', '@SP', 'M=D']   
        Sys_init = list(self.code_C_CALL(len(self.input_data ) + 1, ['call', 'Sys.init', '0'], 'C_CALL'))
        halt = ['(HALT)', '@HALT', '0;JMP']
        system_init_asm = SP_init + Sys_init + halt  
     
        if VMTranslator.IS_directory:
            with open(self.output_file, 'w') as log: 
                for i in range(len(system_init_asm)): 
                    log.write('{}\n'.format(system_init_asm[i])) 
                
                for j in range(len(self.DIR_outputs)):
                    curr_asm_dict = self.DIR_outputs[j]

                    for a in range(len(curr_asm_dict)):  
                        for b in range (len(curr_asm_dict[a]['assembly code'])): 
                            log.write('{}\n'.format(curr_asm_dict[a]['assembly code'][b]))  

        if not VMTranslator.IS_directory:
            with open(self.output_file, 'w') as log: 
                for j in range(len(self.output_dict)):  
                    for k in range (len(self.output_dict[j]['assembly code'])): 
                        log.write('{}\n'.format(self.output_dict[j]['assembly code'][k]))  
        return 

if __name__ == '__main__':  
    """
    Purpose: Develop a VM translater program that translates VM code into executable Hack code
    Input: Source programme is supplied in text file called prog.vm
    Output: Generated source code is written into a text file called prog.asm
    """
    import sys
    import os 
    
    # initialising the Assembler class and I/O files
    VMTranslator = VMTranslator() # initialise the assembler object
    VMTranslator.path = sys.argv[1] # save path provided
    VMTranslator.IS_directory = os.path.isdir(VMTranslator.path) # is this a file or a directory
    
    if not VMTranslator.IS_directory:  
        # initialising the Assembler class and I/O files
        VMTranslator.input_file = sys.argv[1] # initialise the input file
        VMTranslator.output_file = sys.argv[1][:-3] + '.asm'
        VMTranslator.current_file_read = VMTranslator.input_file.split('/')[-1][0:-3]    
        
        # initialise key data structures    
        VMTranslator.parser(VMTranslator.input_file) # load all commands into the input data list
        VMTranslator.initialise_output_dictionary(VMTranslator.input_data) # load the commands into an output dictionary
        VMTranslator.initialise_standard_VMcommand_table() # load standard VM commands and their types
        VMTranslator.initialise_standard_VMtosymbol() # set up the standard VM memory command to assembly symbol map
        VMTranslator.initialise_output_dict(VMTranslator.input_data) # initialise the output dictionary
    
        # begin pre-translation processing 
        VMTranslator.get_VMcode_type(VMTranslator.input_data)# Load VM commmand types in the output dictionary with each command
 
        # begin translating 
        VMTranslator.translate()   
        VMTranslator.write_asm_output()
      
    # save all the VM files in a list
    if VMTranslator.IS_directory:
        outputs = []
        for file_name in sorted(os.listdir(VMTranslator.path)):
            if file_name.endswith(".vm"):
                VMTranslator.file_list.append(file_name) 

                # initialising the Assembler class and I/O files
                VMTranslator.input_file = VMTranslator.path + '/' + file_name # initialise the input file
                VMTranslator.output_file = VMTranslator.path + '/' + os.path.split(VMTranslator.path)[1] + '.asm'
                VMTranslator.current_file_read = VMTranslator.input_file.split('/')[-1][0:-3]
                
                # initialise key data structures    
                VMTranslator.parser(VMTranslator.input_file) # load all commands into the input data list
                VMTranslator.initialise_output_dictionary(VMTranslator.input_data) # load the commands into an output dictionary
                VMTranslator.initialise_standard_VMcommand_table() # load standard VM commands and their types
                VMTranslator.initialise_standard_VMtosymbol() # set up the standard VM memory command to assembly symbol map
                VMTranslator.initialise_output_dict(VMTranslator.input_data) # initialise the output dictionary
                
                # begin pre-translation processing  
                VMTranslator.get_VMcode_type(VMTranslator.input_data)# Load VM commmand types in the output dictionary with each command

                # begin translating 
                VMTranslator.translate()   
                outputs.append(VMTranslator.output_dict)
                

                
        # write to file    
        VMTranslator.DIR_outputs = outputs  
        VMTranslator.write_asm_output() 
        #print(VMTranslator.DIR_outputs) 
        
    
     #for k,v in VMTranslator.output_dict.items(): print(v)
    
   
    
        
 
    


    

     